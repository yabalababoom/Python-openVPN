...
ovpn_dir = os.listdir('C:\\Users\\XXX\\Downloads\\Telegram Desktop\\OpenVPN-Collection-(@proSSH)\\ExpressVPN-(@proSSH) 143')  # 返回该路径下的文件和目录信息,存储在列表ovpn_dir中
# print(ovpn_dir)

lst=[]  # 定义一个空列表,用于存放ovpn文件位置信息

for item in ovpn_dir:  # 遍历ovpn_dir ,将ovpn文件和父目录拼接成一个绝对路径,并存放在lst列表中
    olst = os.path.join('C:\\Users\\XXX\\Downloads\\Telegram Desktop\\OpenVPN-Collection-(@proSSH)\\ExpressVPN-(@proSSH) 143', item)
    lst.append(olst)
# print(lst)
